...
/*				var oDialog = this.getView().byId("SettingsDialog");
			...
			onConfirm: function (oEvent) {
				...
				console.log("Parameters", mParams);
				console.log("groupItem", mParams.groupItem);
				// 当前Method 不对，再查证
				//console.log("groupItem", mParams.getProperty("groupItem"));

				// Apply grouping
				var aSorters = [];
				if (mParams.groupItem) {
					var sGroupKey = mParams.groupItem.getKey();
					console.log("GroupKey", sGroupKey);
					var bDescending = mParams.groupDescending;

					aSorters.push(new sap.ui.model.Sorter(sGroupKey, bDescending, true));
				}

				// Apply sorter
				if (mParams.sortItem) {
					var sSortKey = mParams.sortItem.getKey();
					console.log("SortKey", sSortKey);
					var bDescending = mParams.sortDescending;
					aSorters.push(new sap.ui.model.Sorter(sSortKey, bDescending));
				}
				...
				if (mParams.filterItems) {
					...
					for (var i = 0; i < count; i++) {
						...
						var oFilter = new Filter(oFilterItem.getKey(), sap.ui.model.FilterOperator.EQ, oFilterItem.getText());
			...
			_getColumnHeaders: function () {
				...
				for (var i = 0; i < aColumns.length; i++) {
					...
					//console.log("sId", sColumnID);
					
					var sHeaderText = aColumns[i].getHeader().getText();
					//console.log("HeaderText", sHeaderText);
				...
				//console.log("ColumnHeaders", aColumnHeaders);
			...
			_getCities: function () {
				...
				// 使用 JSON model 
				var sServiceUrl = "https://cors-anywhere.herokuapp.com/"
					            + "http://services.odata.org/V3/Northwind/Northwind.svc/Suppliers";
